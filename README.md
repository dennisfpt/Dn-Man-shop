...
9. **Register / Login / Account** — `/register`, `/login`, and `/account` pages backed by `AuthContext`. Session persists across page reloads via `localStorage`. The Navbar shows a user icon that links to Login (guest) or Account (signed in). **Checkout requires an account**: visiting `/checkout` while signed out redirects to `/login` and returns the shopper to checkout right after signing in, with their name/phone pre-filled.
